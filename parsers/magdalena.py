...
def _parse_block(block: str) -> Dict:
    ...
    # 1) número (en la cabecera "# Orden: ...")
    m = HDR_RE.match(lines[0])
    if not m:
        # por si hay espacios/líneas previas
        for ln in lines[:3]:
            m = HDR_RE.match(ln)
            if m:
                break
    ...
    # 2) fecha imposición: primer YYYY-MM-DD que aparezca (ej. "2021-11-17 - 15:55")
    joined = "\n".join(lines)
...
def parse_magdalena_text(raw_text: str) -> Tuple[List[Dict], str]:
