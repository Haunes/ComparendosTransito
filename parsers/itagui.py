...
def _parse_line(line: str) -> Dict:
    ...
    # campos intermedios opcionales (como Medellín)
    descripcion = None
    fecha_resolucion = None
    valor_interes = None

    # fecha_resolucion si aparece
    mdate = re.search(r"\b(\d{2}/\d{2}/\d{4})\b", middle)
    if mdate:
        fecha_resolucion = mdate.group(1)

    # valor_interes si aparece
    minter = MONEY_RE.search(middle)
    if minter:
        valor_interes = _to_int_money(minter.group(1))

    # descripción si queda texto distinto de “No aplica”
...
def parse_itagui_text(raw_text: str) -> Tuple[List[Dict], str]:
